...
export default function ResetPassword() {
    ...
    const handleReset = async () => {
        ...
        setLoading(true);
        try {
            const response = await fetch('https://slyzah.co.za/api/reset-password/confirm', {
                method: 'POST',
    ...
    if (success) {
        return (
            <View style={styles.container}>
                <View style={[StyleSheet.absoluteFill, { backgroundColor: 'rgba(0, 31, 63, 0.9)' }]} />
                <View style={styles.content}>
                    <View style={styles.solidContainer}>
                        <Text style={{ fontSize: 50, textAlign: 'center', marginBottom: 20 }}>✅</Text>
                        <Text style={[styles.title, { textAlign: 'center' }]}>Password Reset Successful</Text>
